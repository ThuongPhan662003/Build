...
//process the user data and save it to the database
	if (count($_POST) > 0 && strlen($_POST["persistformvar"]) == 0) {
		...
		//build the xml dialplan
			...
			{
				$dialplan_xml .= "		<action application=\"set\" data=\"effective_caller_id_name=".$effective_caller_id_name."\"/>\n";
			}
			if ($effective_caller_id_number != "") 
			{
				$dialplan_xml .= "		<action application=\"set\" data=\"effective_caller_id_number=".$effective_caller_id_number."\"/>\n";
			}
			// end quang
			$dialplan_xml .= "		<action application=\"lua\" data=\"call_flow.lua\"/>\n";
			$dialplan_xml .= "	</condition>\n";
		...
		//build the dialplan array
			...
			// Quang insert
			// $array["call_flows"][$i]["call_flow_effective_caller_id_name"] = $effective_caller_id_name;
			// $array["call_flows"][$i]["call_flow_effective_caller_id_number"] = $effective_caller_id_number;
			// End quang
			...
			$sql = "delete from v_dialplans ";
				...
				//dialplan group 2
					...
					if ($effective_caller_id_name !="")
					{
						...
						$dialplan->dialplan_detail_tag = 'action'; //condition, action, antiaction
						$dialplan->dialplan_detail_type = 'set';
						$dialplan->dialplan_detail_data = 'effective_caller_id_name='.$effective_caller_id_name;
						//$dialplan->dialplan_detail_break = '';
						//$dialplan->dialplan_detail_inline = '';
						$dialplan->dialplan_detail_group = '2';
						$dialplan->dialplan_detail_order = '25';
						$dialplan->dialplan_detail_add();
						unset($dialplan);
					}
					...
					if ($effective_caller_id_number !="")
					{
						...
						$dialplan->dialplan_detail_tag = 'action'; //condition, action, antiaction
						$dialplan->dialplan_detail_type = 'set';
						$dialplan->dialplan_detail_data = 'effective_caller_id_number='.$effective_caller_id_number;
						//$dialplan->dialplan_detail_break = '';
						//$dialplan->dialplan_detail_inline = '';
						$dialplan->dialplan_detail_group = '2';
						$dialplan->dialplan_detail_order = '30';
						$dialplan->dialplan_detail_add();
						unset($dialplan);
					}
					// end quang
...
//pre-populate the form
	if (is_array($_GET) && $_POST["persistformvar"] != "true") {
		...
		$sql = "SELECT dialplan_detail_data FROM v_dialplan_details  join v_call_flows ";
		$sql .= "ON v_dialplan_details.dialplan_uuid = v_call_flows.dialplan_uuid ";
		$sql .= "WHERE call_flow_uuid= '$call_flow_uuid' AND v_dialplan_details.dialplan_detail_data LIKE '%effective_caller_id%'";
		$prep_statement = $db->prepare(check_sql($sql));
		$prep_statement->execute();
		$result = $prep_statement->fetchAll(PDO::FETCH_NAMED);
		// $effective_caller_id_name = explode("=",$result[0]["dialplan_detail_data"])[1];
		// $effective_caller_id_number = explode("=",$result[1]["dialplan_detail_data"])[1];
		if(count($result) > 0) {
			foreach($result as $rs) {
				if(explode("=",$rs["dialplan_detail_data"])[0] == "effective_caller_id_name") {
					
					$effective_caller_id_name =explode("=",$rs["dialplan_detail_data"])[1];
				}
				if(explode("=",$rs["dialplan_detail_data"])[0] == "effective_caller_id_number") {
					$effective_caller_id_number =explode("=",$rs["dialplan_detail_data"])[1];
				}
			}
		}
		unset ($prep_statement);
...
//get the recordings
	...
	function sound_select_list($var, $name, $description_name, $load_sound=false) {
		...
		echo "<select name='".escape($name)."' class='formfld' ".((if_group("superadmin") || if_level("admin")) ? "onchange='changeToInput(this);'" : null).">\n";
