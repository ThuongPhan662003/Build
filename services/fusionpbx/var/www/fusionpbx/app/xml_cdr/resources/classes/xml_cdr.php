	...
	class xml_cdr {
		...
		public function user_summary() {
			...
			//calculate the summary data
				...
				$sql .= " AND c.direction = 'outbound' AND c.answer_stamp is not null AND c.bridge_uuid is not null \n";
				$sql .= ") \n";
				$sql .= "AS outbound_calls, \n";
				// QUANG
				
				$sql .= "COUNT(c.duration) \n";
				$sql .= "FILTER ( \n";
				$sql .= " WHERE c.extension_uuid = e.extension_uuid \n";
				$sql .= "AND c.duration >= 60 \n";
				$sql .= ") \n";
				$sql .= "AS call_time, \n";
				// END
				$sql .= "SUM(c.billsec) \n";
		...
		public function download() {
			...
			if ($alway_download || permission_exists('xml_cdr_view')) {
